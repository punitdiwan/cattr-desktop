...
app.once("ready", async () => {
  ...
  const loadPage = (page) => {
    ...
    sentryConfiguration.enabled =
      config.sentry.enabled && userPreferences.get("errorReporting");

    // Encode to JSON, then encode to URIEncoded
    sentryConfiguration = encodeURIComponent(
      JSON.stringify(sentryConfiguration)
    );
  ...
  window.webContents.on("will-navigate", (event, url) => {
    ...
    if (!WEBCONTENTS_ALLOWED_PROTOCOLS.has(targetUrl.protocol)) return;
